...
#TABELA BASE
brutosntroglo = read.csv("Geralntroglo_quadrantes_r_limpo.csv", sep=";", header = T) %>%
  replace(is.na(.), 0)
...
comunidade_nt = brutosntroglo %>%
  select(ID, Blattidae_sp2 : Novamundoniscus_sp1)

row.names(comunidade_nt) = comunidade_nt$ID
comunidade_nt = comunidade_nt[,-1]
write.table(comunidade_nt, "Comunidade_NT.csv", row.names = T, sep = ";")
...
ambiente_nt = brutosntroglo %>%
  select(ID, Sector, Cave, Basin, Hidro, Distance, Subs.Div., 
...
#chart.Correlation(ambiente_nt[,-c(1,2, 3, 4)], histogram = TRUE, method = c("spearman"))
#dev.off()  
#remover subs.div.
...
#DBRDA
db0_nt = capscale(diss_jaccard_nt ~ 1, data = ambiente_nt[,-c(1,3,4,6)])
db_nt = capscale(diss_jaccard_nt ~ ., data = ambiente_nt[,-c(1,3,4,6)])
...
#MODELO FINAL DBRDA 
db_final_nt = capscale(diss_jaccard_nt ~ Cave + Distance + Shelter.Av., data = ambiente_nt[,-c(1, 3, 4,6)])
...
#######GRAFICO DBRDA#### 
sites_scores_nt = as.data.frame(db_nt$CCA$u[, 1:2])
colnames(sites_scores_nt) = c("CAP1", "CAP2")

env_vectors_nt = as.data.frame(db_nt$CCA$biplot[, 1:2])
...
cores_cavernas <- c(
  "Cânion da Baixa Verde Cave" = "#2ca02c",   # Azul
  "Cedrão Cave"                = "#2ca02c",   # Verde (mantida da original)
  ...
  "Boqueirão Cave"             = "#2ca02c",   # Verde-amarelado
  "Couve Flor Cave"            = "#17becf",   # Azul-petróleo
  "São Geraldo Cave"           = "#ffbb78",   # Laranja-claro
  "Tunel 2 Cave"               = "#2ca02c",   # Verde-claro
  "Geraldo Cruz Cave"          = "#2ca02c",   # Vermelho-claro
  "Duas Cobras Cave"           = "#9467bd",   # Lilás
  "Olho D'Água do Cumbra Cave" = "#17becf",   # Bege
  "Fenda Oblíquoa Cave"        = "#2ca02c",   # Rosa-claro
  "Tunel 1 Cave"               = "#dbdb8d",   # Amarelo-claro
  "Pajeú Cave"                 = "#2ca02c",   # Azul-claro
  "Grota Cave"                 = "#ff7f0e",   # Azul-escuro
  "Salobro Cave"               = "#ff7f0e",   # Verde-escuro
  "Cedrículo Cave"             = "#2ca02c",   # Marrom-claro
  "Cinquentona Cave"           = "#843c39",   # Vinho
  "Escrevidinha Cave"          = "#2ca02c",   # Roxo-escuro
  "Cristal Cave"               = "#2ca02c"    # Azul-claro pastel
)
...
posicoes_nomes <- data.frame(
  ...
  x_pos = c(0.018598500, -0.018963575, -0.031859698, -0.082223747, 0.019997723,
            0.032997723, 0.015563670, 0.044196128, 0.115990325, -0.02559698,
            0.02267548, 0.004438950, -0.095114644, 0.147605502, -0.002160614,
            0.042303449, -0.062223747, -0.010748017, -0.001269953, 0.155959074,
            -0.038531059, 0.076125170, 0.089574485, 0.008000000),
  y_pos = c(-0.05473087, -0.03002368, -0.04565082, 0.01354468, -0.08229990,
            -0.24029990, -0.10992898, 0.04544796, 0.06612125, -0.08915118,
            -0.18011712, -0.03211281, 0.04033663, 0.09702948, 0.05933992,
            -0.00259304, 0.07596924, -0.06709902, 0.02174545, 0.03545552,
            -0.00259304, -0.11844401, 0.01162970, -0.14127253)
) %>%
...
ggplot() +
  ...
  geom_text_repel(data = env_vectors_nt[env_vectors_nt$Variable == "Shelter.Av.",],
                  aes(x = CAP1_scaled * 1.1, y = CAP2_scaled * 2, 
                      label = Variable),
                  color = "black", fontface = "bold", size = 6) +
  ...
  labs(title = "Microscale - dbRDA - Non-troglobitic species",
...
ggsave("DBRDA_NaoTroglos_Quad.tiff", width = 14, height = 10, dpi = 300, bg = "white")

###################################################################################
#########################ENTENDENDO AS RELAÇÕES ESPACIAIS#####################

coord_quad_nt = read.csv("quadrantes_utm_ID.csv", sep=";", header = T)
coord_quad_nt = coord_quad_nt[linhas_validas_nt, ]

coords_matrix_nt = cbind(coord_quad_nt$x, coord_quad_nt$y)
...
ggsave("Mantel_NaoTroglos_Quad_teste.tiff", width = 14, height = 10, dpi = 300, bg = "white")


######################PARTIÇÃO DA VARIAÇÃO###################################
###DADOS AMBIENTAIS###
X_env = ambiente_nt[, c("Distance", "Shelter.Av.")]

###GERANDO POLINOMIOS DE TENDENCIA ESPACIAL###

#COORDENADAS
coords = coord_quad_nt[, c("x", "y")]
rownames(coords) = coord_quad_nt$ID
...
vp = varpart(comm.bin_nt, ~ Cave + Distance + Shelter.Av., # Fórmula para Environment
...
rda_env = rda(comm.bin_nt ~ Cave + Distance + Shelter.Av. + Condition(MEM1 + MEM2),
...
rda_spa = rda(comm.bin_nt ~ MEM1 + MEM2  + Condition(Cave + Distance + Shelter.Av.),
               data = cbind(ambiente_nt, X_spa))
set.seed(125)
anova(rda_spa) 
...
comunidade=brutosntroglo%>%
  dplyr::select(ID, Blattidae_sp2 : Novamundoniscus_sp1)
...
#EFEITO DA DISTANCIA
pred_dist = ggpredict(m1, terms = c("Distance"))
